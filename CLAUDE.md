...
- Прогресс долгих операций (`fetch`/`download`/`pipeline`): по умолчанию — Rich-бар на stderr; `-v`/`-vv` — бар отключается в пользу структурных логов; `--quiet` и не-TTY — тишина (Rich сам не рендерит бар на не-TTY).
