...
│   └── progress.py          — RichProgressReporter: Rich-бары на stderr (реализация
│                               ProgressReporterProtocol); transient, подписи фаз «Страницы»/
│                               «Сканы»/«Иллюстрации»
...
- Прогресс долгих операций (`fetch`/`download`/`pipeline`): по умолчанию — Rich-бар на stderr; `-v`/`-vv` — бар отключается в пользу структурных логов; `--quiet` и не-TTY — тишина (Rich сам не рендерит бар на не-TTY); фаза, целиком взятая из кэша, оставляет строку «<Фаза>: N из кэша» вместо бара.
