...
Чистая типизированная логика очистки. Парсинг реализован на регулярных
выражениях; структурирован так, чтобы при необходимости заменить его на
selectolax-парсер, не меняя публичный контракт функций.
"""
...
def normalize_text(text: str | None) -> str:
    ...
    НЕ срезает ведущее число: удаление номера страницы выполняется на уровне
    извлечения блоков (фильтр ``page-no``), а не здесь. Иначе значимое ведущее
    число (например ``"1941\\nгод"``) было бы потеряно.
    """
