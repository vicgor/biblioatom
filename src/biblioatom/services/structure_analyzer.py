...
Перенос ядра доменных знаний из legacy ``convert.py`` с типизацией на
Pydantic-модели (``BookElement``/``PageModel``/``StructuredChapter``/
``StructuredDocument``). Эвристики заголовков/авторов сохранены почти as-is —
это самый ценный актив проекта.

Исправленные при переносе баги (см. план миграции, раздел 5):

* ``is_probable_author_line`` теперь распознаёт дефисные фамилии
  (например ``"Боровик-Романов А. С."``).
* ``split_by_toc`` больше не теряет страницы с номером меньше ``toc[0].page``
...
def parse_embedded_content(raw: str | dict[str, object] | None) -> EmbeddedContent:
    ...
    ``raw`` — либо JSON-строка, либо уже распарсенный словарь (совместимость с
    legacy-форматом). При ошибке разбора JSON возвращается ``valid=False`` с
    исходным текстом в ``pagetext``.
    """
...
def is_probable_author_line(text: str) -> bool:
    ...
    Распознаёт дефисные фамилии вида ``"Боровик-Романов А. С."`` — legacy-версия
    ошибочно возвращала для них ``False``, потому что регулярка фамилии не
    допускала внутренний дефис.
    """
