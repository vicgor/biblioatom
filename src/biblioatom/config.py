...
class ScanExtractionSettings(BaseModel):
    ...
    * паддинг кропа в пикселях — ``crop_padding``.
    ...
    margin_px: int = Field(
        ...
        ge=0,
        description=(
            "Ширина полей страницы для оценки уровня белого"
            " и исключения колонтитулов (пикселей)."
        ),
    )
    ...
    small_region_area_ratio: float = Field(
        ...
        le=1.0,
        description=(
            "Порог площади (доля от страницы) ниже которого"
            " fallback-1 объект считается 'мелким'."
        ),
    )
