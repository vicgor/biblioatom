...
Без флагов долгие операции показывают прогресс-бар (stderr); с `-v`/`-vv` вместо бара выводятся логи; `--quiet` и не-TTY отключают и то и другое; фаза, целиком взятая из кэша, вместо бара печатает строку «<Фаза>: N из кэша».
